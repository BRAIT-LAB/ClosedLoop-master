...
dqn = DQN()
state = torch.load(r'../RL/model.pth')
dqn.eval_net.load_state_dict(state)
action = ['减少', '增加']


def RL_num(num, old_attention, attention):
    attention = int(attention) * 0.01
    a = dqn.choose_action([num, old_attention, attention])
    if a == 0 and num > 5:
        num = num - 1
    elif a == 1 and num > 15:
        num = num - 1
